...
void DoubleRegisters::init(uint8_t data_pin, uint8_t latch_pin, uint8_t clock_pin, uint8_t clear_pin) {
  _data_pin = data_pin;
  ...
  digitalWrite(_clear_pin, HIGH); //inactive when HIGH
}
...
void DoubleRegisters::reset() {
  digitalWrite(_clear_pin, LOW); //LOW to be held for ~5ns
