...
void determineDirection() {
  ...
  if ((distances_mm[1] > distances_mm[0]) && (distances_mm[1] > distances_mm[2])){ //go forward
    curr_direction = 'F';
    digitalWrite(NO_DIRECTION_LED_PIN, LOW);
  
  } else if (distances_mm[0] > distances_mm[2]) { //more space on the left...
    if (!is_left_right_in_tolerance) { //only change direction if the distances are pretty close to each other
      curr_direction = 'L';
    } else {
      curr_direction = 'F';
    }
  ...
  } else if (distances_mm[2] > distances_mm[0]) { //more space on the right...
    if (!is_left_right_in_tolerance) {//only change direction if the distances are pretty close to each other
      curr_direction = 'R';
    } else {
      curr_direction = 'F';
    ...
    //dunno, flash a light????
    digitalWrite(NO_DIRECTION_LED_PIN, HIGH);
  }
}
