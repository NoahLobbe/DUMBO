...
void MotorSystem::_writeSpeed(uint8_t pwm_pin, uint8_t speed_val) {
  if (_PRINT_RESULTS) Serial.println("_PWM_SPEED_ENABLED: " + String(_PWM_SPEED_ENABLED));
  
  if (_PWM_SPEED_ENABLED) {
    if (_PRINT_RESULTS) Serial.println("analog writing!!!");
