...
void loop() {
  if (Serial.available() > 0) {
    String input = Serial.readString(); //get user input from serial monitor
    char motor_select = input[0];
    char command = input[1];
    int speed_value = (input.substring(2).toInt()); //converting types
    String text = "\nYou inputted..."
    ...
    if (motor_select == 'D') {
      ...
      Serial.println("...unrecognized motor_select, '" + String(motor_select) + "'");
    }
  }
