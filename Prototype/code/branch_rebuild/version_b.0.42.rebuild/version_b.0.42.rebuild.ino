 ...
 * 
 * Version: b.0.42.rebuild
 * Version made: 13 September 2023, Noah Lobbe
...
#define OBJECT_AVOIDANCE_DISTANCE_THRESHOLD_MM 600
#define SLOW_DOWN_OBJECT_DISTANCE_THRESHOLD_MM (200 + OBJECT_AVOIDANCE_DISTANCE_THRESHOLD_MM) 
#define DIST_TOLERANCE_MM 10
...
void loop() {
  ...
  uint16_t distance_mm = (SPEED_OF_SOUND_MM_PER_S * duration_uS * 1e-6) / 2; //conversions done inline to prevent inaccuracies through rounding
  

  ////outputs
  if (PRINT_RESULTS) {
    Serial.print("uS: " + String(duration_uS) + ", distance(mm): " + String(distance_mm) + ", prev_dist: " + String(prev_distance_mm) + " | ");
  }
  ...
  bool not_inside_distance_tolerance = !withinTolerance(distance_mm, prev_distance_mm, DIST_TOLERANCE_MM);
  digitalWrite(EXTRA_LED, !not_inside_distance_tolerance);
  Serial.print("\tnot_inside_distance_tolerance: " +String(not_inside_distance_tolerance));
  ...
  if ((distance_mm <= OBJECT_AVOIDANCE_DISTANCE_THRESHOLD_MM) && (distance_mm > 0)) { //if distance_mm just means pulseIn(...) timed out
    ...
    if ((distance_mm <= SLOW_DOWN_OBJECT_DISTANCE_THRESHOLD_MM) && (distance_mm > 0)) {
      vehicle_speed = 192;
      digitalWrite(MOTION_LED_PIN, HIGH);
    } else {
      vehicle_speed = 255;
